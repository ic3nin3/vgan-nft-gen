...
#parser.add_argument("-t", "--text_prompt", dest = "text_prompt")
#parser.add_argument("-i", "--image_path", dest = "image_path")
#args_prompt = parser.parse_args()

import random
rnd = random.choice(range(1000))
texts = sys.argv[1]
width =  599
height =  599
model = "vqgan_imagenet_f16_16384"
images_interval =  50
init_image = sys.argv[2]
target_images = ""
seed = rnd
max_iterations = 50
input_images = ""
...
jsonString = json.dumps(args_dict)
img_name = os.path.basename(init_image).split('.')[0]

dest = sys.argv[3]
numb = sys.argv[4]
path = './metadata/' + img_name + '/' + texts[0] + '/'
path = './metadata/'
if not os.path.exists(path):
    os.makedirs(path)
with open(path + dest + numb + '.json', 'w') as f:
...
### two deleted functions
@torch.no_grad()
def checkin(i, losses):
    losses_str = ', '.join(f'{loss.item():g}' for loss in losses)
    tqdm.write(f'i: {i}, loss: {sum(losses).item():g}, losses: {losses_str}')
    out = synth(z)
    TF.to_pil_image(out[0].cpu()).save('./output/' + numb + dest)
